...
<?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
	...
	<div class="vitals">
		<img class="contact-picture" src="<?php $wpcm_image_path = get_post_meta($post->ID, "image_path_wpcm_value", true); if ($wpcm_image_path == '') { echo bloginfo('template_directory') . '/images/contact-default.png'; } else { echo get_post_meta($post->ID, "image_path_wpcm_value", true); } ?>" alt="<?php echo get_post_meta($post->ID, "first_name_wpcm_value", true); ?> <?php echo get_post_meta($post->ID, "last_name_wpcm_value", true); ?>" />
		<p>
		  <span class="organization"><?php echo get_post_meta($post->ID, "organization_wpcm_value", true); ?></span>
			<span class="name"><?php echo get_post_meta($post->ID, "first_name_wpcm_value", true); ?> <?php echo get_post_meta($post->ID, "last_name_wpcm_value", true); ?></span>
...
<div id="sidebar">
	...
	<h3 class="site-subtitle">Search Contacts</h3>
	<?php include (TEMPLATEPATH . '/searchform.php'); ?>

	<h3 class="site-subtitle">Tags</h3>
	<p><?php wp_tag_cloud('smallest=10&largest=18&unit=px&format=flat'); ?></p>
	
	<div id="ind-contact-meta">
		<p>
			<?php edit_post_link('Edit This Contact'); ?> 
			<a target="_blank" id="location-link" href="http://maps.google.com/maps?f=q&amp;hl=en&amp;q=<?php echo get_post_meta($post->ID, "address_1_wpcm_value", true); ?>,<?php echo get_post_meta($post->ID, "city_wpcm_value", true); ?>,<?php echo get_post_meta($post->ID, "state_wpcm_value", true); ?>,<?php echo get_post_meta($post->ID, "zip_wpcm_value", true); ?>">View Location</a>
			<span class="clearer"></span>
		</p>
	</div>
